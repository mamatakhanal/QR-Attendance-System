        ...
        <div class="dropdown">
          <a href="#" class="d-flex align-items-center text-decoration-none dropdown-toggle" role="button" aria-expanded="false" data-bs-toggle="dropdown">
            <div class="bg-dark text-white rounded-circle d-flex justify-content-center align-items-center"
              style="width:32px; height:32px; font-size:14px;"> {{ strtoupper(substr(session('student_name'), 0, 1)) }} </div>
            <span class="ms-2 fw-semibold text-dark"> {{ \Illuminate\Support\Str::of(session('student_name') ?? 'Student')->before(' ') }} </span>
          </a>
          <ul class="dropdown-menu dropdown-menu-end rounded-3 shadow-lg" style="min-width:200px;">
            <li> <a class="dropdown-item text-dark small" href="{{ route('student.profile') }}"> <i class="bi bi-person-circle me-2"></i> Profile </a> </li>
            <li>
