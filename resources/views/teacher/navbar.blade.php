      ...
      <div class="d-flex align-items-center gap-4">
        ...
        <form method="GET" action="{{ url()->current() }}">
          <div class="search-box position-relative mt-3" style="max-width:300px;">
            ...
            <input type="text" id="searchInput" name="search" value="{{ request('search') }}" class="form-control form-control-sm rounded-3 ps-4 py-2" placeholder="Search.....">
          </div>
        </form> --}}
        ...
        <div class="dropdown">
          <a href="#" class="d-flex align-items-center text-decoration-none dropdown-toggle" role="button" aria-expanded="false" data-bs-toggle="dropdown">
            <div class="bg-dark text-white rounded-circle d-flex justify-content-center align-items-center"
              style="width:32px; height:32px; font-size:14px;"> {{ strtoupper(substr(session('teacher_name'), 0, 1)) }} </div>
            <span class="ms-2 fw-semibold text-dark"> {{ \Illuminate\Support\Str::of(session('teacher_name') ?? 'Teacher')->before(' ') }} </span>
          </a>
          <ul class="dropdown-menu dropdown-menu-end rounded-3 shadow-lg" style="min-width:200px;">
            <li> <a class="dropdown-item text-dark small" href="{{ route('teacher.profile') }}"> <i class="bi bi-person-circle me-2"></i> Profile </a> </li>
            <li>
              <hr class="dropdown-divider my-1">
            </li>
            <li>
              <form action="{{ route('teacher.logout') }}" method="POST" class="m-0 p-0">
                @csrf
